...
class SWEM():
    ...
    def get_word_embeddings(self, text):
        ...
        for word in self.tokenizer.tokenize(text):
            if word in self.vocab:
                ...
                vectors.append(
                    np.random.uniform(self.oov_initialize_range[0],
    ...
    def concat_average_max_pooling(self, text):
        word_embeddings = self.get_word_embeddings(text)
        return np.r_[np.mean(word_embeddings, axis=0),
                     np.max(word_embeddings, axis=0)]
    ...
    def hierarchical_pooling(self, text, n):
        ...
        window_average_pooling_vec = [
            np.mean(word_embeddings[i:i + n], axis=0)
